...
def train(cfg: "DictConfig"):  # noqa: F821
    ...
    for i in range(cfg.collector.n_iters):
        # Determine number of agents for this round
        num_agents = random.randint(
            cfg.env.get("min_agents", 4), 
            cfg.env.get("max_agents", 8)
        )
        
        # Get the environment with exactly this many agents
        env = env_factory.get_env(num_agents)
        
        # Create collector with this environment
        collector = SyncDataCollector(
            env,
            policy,
            device=cfg.env.device,
            storing_device=cfg.train.device,
            frames_per_batch=cfg.collector.frames_per_batch,
            total_frames=cfg.collector.frames_per_batch,  # Only collect one batch
            postproc=DoneTransform(reward_key=env.reward_key, done_keys=env.done_keys),
        )
