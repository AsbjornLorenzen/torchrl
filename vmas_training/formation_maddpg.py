...
@hydra.main(version_base="1.1", config_path="", config_name="maddpg")
def train(cfg: DictConfig):  # noqa: F821
    ...
    # Policy (Actor) - Modified to use nn.Sequential for structural similarity with MAPPO,
    # but output size is kept for TanhDelta (mean only)
    actor_net = nn.Sequential(
        MultiAgentMLP(
            n_agent_inputs=env.observation_spec["agents", "observation"].shape[-1],
            # Output size is action_spec size for TanhDelta (mean)
        ...
        # NormalParamExtractor is not used for TanhDelta
    )
    ...
    policy = ProbabilisticActor(
        ...
        in_keys=[("agents", "param")], # TanhDelta takes 'param' (mean)
        ...
        return_log_prob=False, # MADDPG typically does not use log_prob for policy gradients
    )
    ...
    # Critic - Underlying network uses MultiAgentMLP with similar structure to MAPPO,
    # but inputs observation AND action as required by MADDPG Q-critic
    module = MultiAgentMLP(
        # Input size includes both observation and action
        n_agent_inputs=env.observation_spec["agents", "observation"].shape[-1]
    ...
    value_module = ValueOperator(
        module=module,
        # Critic takes observation and action in MADDPG
